...
// 1차 분류 셀렉트 박스에 데이터 삽입, ($("")대신 cate1Select를 사용하겠다는 의미입니다.)
var cate1Select = $("select.category1") // select.catrgory1은 데이터를 넣어서 사용될 요소이기 때문에 값 자체가 없는게 맞다.

for(var i = 0; i < cate1Arr.length; i++) {

	// cate1Arr에 저장된 값을 cate1Select에 추가
	// cate1Select.append( [ 내용 ] ); = $("select.category1").apped( [ 내용 ] )
...
$(document).on("change", "select.category1", function(){
	...
	*/
	...
	/*
		다른 1차 분류를 선택했을 경우, 2차 분류가 중복출력되는 문제가 있습니다. 
		1차 분류를 다른걸로 선택했을 때, [2차 분류의 내용을 모두 지운뒤],
		 1차 분류에 맞는 2차 분류를 추가하는 방식으로 변경해야겠습니다.
	*/

	// cate2Select의 값을 제거함(초기화), [2차 분류의 내용을 모두 지운뒤]
	...
	$("option:selected", this).each(function(){
		...
		for(var i = 0; i < cate2Arr.length; i++) {
			...
			if(selectVal == cate2Arr[i].cateCodeRef) { // selectVal = cate1Select의 저장된 값
				cate2Select.append("<option value='" + cate2Arr[i].cateCode + "'>"
									+ cate2Arr[i].cateName + "</option>");
			}
		}		
